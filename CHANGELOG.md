...
- **The restock E2E specs were only ever testing the seed, even with the backend up.** They navigated to the hardcoded seed id `store-002`. With portfolio_api running, the real fleet comes back as UUIDs, the API 404s for `store-002`, and the BFF quietly falls back to the seed — so the specs passed identically whether or not the backend worked, which makes them useless for catching an integration regression. That is the same blind spot that let a missing `/stores/:id/sales` endpoint sit unnoticed: the fallback that keeps the demo alive also hides whether the real path works. They now read a store id off the fleet page, so they run against whatever is actually serving. Verified both ways — six real restocks through Postgres with the API up, and the seed path with it down.
