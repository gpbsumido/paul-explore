...
- **The first thing a hiring manager saw was a slot machine.** v4 was fun to build and it answered a question nobody visiting had asked. `/` now renders a page with one job: the role said out loud on the page and not just in the metadata, the whole ten-trait craft matrix with every evidence link intact, six featured apps, three write-ups, and a way to get in touch. The slot machine is one click away at `/discover` and still runs exactly as it did.
...
- **Ten trait accents, one page accent.** Each craft trait keeps its own colour, and all ten only ever reach `SpotlightCard`'s accent prop, where they tint glass by five percent and drive the cursor glow. None touches text, so everything a reader acts on is verdigris and the trait colours behave like a legend instead of a second palette.
- **The 3D object is garnish and is built to be dropped.** A wireframe torus knot in a geodesic cage, code-built with no model file, mounted after first paint behind `next/dynamic` with SSR off and inside the same `ModelLazyMount` the Toronto world uses. Three conditions send it to a seeded SVG instead: reduced motion, no WebGL context, and a screen at or under 640px. The context check asks a canvas rather than feature-detecting the constructor, which is also why jsdom exercises the fallback for free.
