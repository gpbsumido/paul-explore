...
- **And the sets *list* page, which is where it moved next.** Removing the detail page's build-time fetch pushed the same failure one route over: `/tcg/pokemon/sets` renders at build under its own `revalidate`, already caught a failing fetch, and then read `serie.sets.length` and `set.cardCount.official` blind — so a fetch that *succeeded* and returned a series not yet filled in ended the export just the same. Both are optional now, a missing count showing a dash. Three tests cover it, and they fail with the same `Cannot read properties of undefined` without the guards.
