...
- polished the v3 landing graph. A batch of fixes:
  ...
  - **Flat view — progressive disclosure.** The flat view now opens collapsed to just the root and its section headers (Apps + the thought categories). Hovering a header reveals that section's items; it stays open after you move away and only collapses when you hover a different header. Edges and the canvas height follow what's shown.
