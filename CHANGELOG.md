...
- **The 3D object cycles through the work.** Every three seconds the wireframe in the geodesic cage crossfades into another shape that stands for something here: the knot for the codebase, a low-poly globe for `/world`, a ball for the NBA console, a card for the TCG browser, an octahedron for the v3 graph, a reel for the slot machine. The next shape is drawn at random but never repeats the current one, the clock lives in `useFrame` so the cycle pauses offscreen with everything else, and the same three conditions as before send the whole thing to a seeded SVG: reduced motion, no WebGL context, a screen at or under 640px.
- **The header is pinned and the second bar is gone.** Theme, settings and the auth call to action stay reachable at any scroll depth on a bar with its own backdrop, and the signed-in account strip that used to sit under the hero is deleted because the header menu already carried those routes. `AnimatedNumber` also rounds before it formats, since a count-up interpolates through floats and `toLocaleString` was sprinkling decimals across the proof strip mid-count.
