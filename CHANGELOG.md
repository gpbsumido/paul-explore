...
- **Found, not fixed: an `aria-hidden` marquee clone that still holds focusable buttons.** It lives in the shared component library, not here. Hiding something from assistive tech while leaving it in the tab order is worse than not hiding it, because a keyboard user lands on a control a screen reader says does not exist.
