...
- **A pseudonymous visitor id, so fairness limiting and the audit trail have something to key on.** The shared service token answers *can this caller write at all*; it never answered *which visitor is this*, so every operator request reached the API from this server and rate limits saw one caller for the entire internet — one person in a loop could have started returning 429s to everybody. Middleware now issues an opaque httpOnly cookie on first contact with the operator routes, and forwards it to the API alongside the service token. Nothing about the person goes into it: no fingerprint, no IP hash, just a random value the server invents. Limits are now per visitor, and a restock records `anonymous:v_…` instead of the same hardcoded `operator@smartstore.example` for everyone.
