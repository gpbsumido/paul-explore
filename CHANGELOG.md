...
- **The feature tours now run on the published design-system `GuidedTour`.** The tour engine I built here became a `@paul-portfolio/react` primitive; this consumes it back. `FeatureTour` renders `GuidedTour` from the package (react `^0.7.0`, css `^0.10.0`, whose `guided-tour.css` rides in through the `components.css` already imported in `globals.css`) for the Fantasy, Pokémon TCG, operator, vitals, and design-system tours. Their consent step now uses the primitive's Skip / Next controls rather than the app's No-thanks / Show-me-around. ZeroProof stays on the app's local engine for now, because its steps switch tabs through a per-step `onEnter` the published primitive doesn't carry yet — `FeatureTour` keeps a tour with any `onEnter` step on the local engine and routes the rest to the package. Once the onEnter-capable version publishes, ZeroProof folds in too and the local overlay goes away. Covered by the existing per-tour tests, updated for the primitive's controls. The design-system showcase gains a `GuidedTour` card (its catalog integrity check requires every package export to be documented and previewed).
