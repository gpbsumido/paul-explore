...
A fleet management dashboard for smart micro-retail stores (lobby fridges, vending machines, kiosks) at `/operator`. The fleet overview shows store cards sorted severity-first (offline > degraded > healthy), an alert summary banner with fleet-wide critical/warning counts, fleet stats (stores needing attention, low-stock items, average inventory health), and a collapsible analytics section with fleet health donut, alert trend area chart, and per-store inventory comparison bars. Status filter pills and name search narrow the view. Each store card shows status badge, alert counts, inventory health bar, and sensor freshness timestamp.

Store detail pages at `/operator/stores/[storeId]` have four tabs synced to `?tab=` URL param: Inventory (stock bars, 7-day sparklines, per-item restock with optimistic updates), Alerts (severity-sorted with category icons, per-alert dismiss, severity filter pills), Activity (chronological event feed with type-coded icons), and Planogram (CSS grid shelf layout with sensor match status). Quick action buttons handle bulk restock, bulk alert dismiss, and force sensor refresh with confirmation modals and toast notifications.

The data layer uses tiered polling — alerts every 15s, store list every 30s, inventory every 60s — all with `staleTime: 0` and `refetchOnWindowFocus: true`. Mutations use TanStack Query's optimistic update lifecycle (snapshot, apply, rollback on error). A data freshness system shows green/amber/red indicators based on sensor reading age, with a `SensorOfflineCallout` banner when sensors haven't reported in 30+ minutes. All data is in-memory mock — no external backend needed.
...
Picks save automatically via a debounced PUT. A `userHasPickedRef` starts false and flips on first interaction — the save effect bails early until that flag is set, so loading server data on mount never triggers a spurious write. A `SaveIndicator` component shows "Saving..." and "Saved" states with animated enter/exit transitions.

Series cards and the Finals card show team logos alongside abbreviations via `nbaTeamColors.ts`, which maps team IDs to brand colors and CDN logo URLs. The Finals card adds two tiebreaker inputs: combined score of the last game and Finals MVP.

A share button copies a direct link to the bracket. The leaderboard rows are clickable — visiting `?view=<sub>` loads any user's bracket in read-only mode, with that user's leaderboard row highlighted in blue. The `/api/nba/playoffs/picks/[sub]` route serves picks for any Auth0 sub. The current user's row highlights in orange; `currentUserSub` comes from the `/api/me` query. The leaderboard response is cached at `s-maxage=300`.
