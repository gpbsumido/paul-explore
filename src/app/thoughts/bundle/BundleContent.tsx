...
export default function BundleContent() {
  return (
    <ThoughtLayout
      ...
      intro={
        <>
          How @next/bundle-analyzer exposed Auth0&apos;s client SDK shipping to
      ...
      chat={
        ...
        <ChatThread>
          ...
          <Sent pos="first">
            ...
            JavaScript bundle as a treemap — each rectangle is a module, sized
            by how many bytes it contributes. you run it once after a build and
            it opens an HTML report in your browser
          </Sent>
          <Sent pos="last">
...
export default withBundleAnalyzer(nextConfig);`}
          ...
          <Sent pos="last">
            ...
            Turbopack by default for builds, and the analyzer does not work with
            Turbopack — it throws an error if you omit it
          </Sent>
          ...
          <Sent pos="middle">
            ...
            component can call <code>useUser()</code> to read the name and email
            without props
          </Sent>
          <Sent pos="last">
...
// what useUser() call count was: 0`}
          ...
          <Sent pos="first">
            <code>jose</code> — JWT parsing and verification. several hundred
            KB. this is the main offender
          </Sent>
          <Sent pos="middle">
...
// layout became synchronous — no more async function`}
          ...
          <Sent pos="last">
            ...
            <code>auth0.getSession()</code> directly and passes the data down as
            props. for API routes, each handler calls{" "}
...
// throws if not authenticated -> 401`}
          ...
          <Sent pos="middle">
            ...
            component, you&apos;d need to either pass it down as a prop or put
            it in your own context — a thin wrapper that you control, that only
          ...
          <Sent pos="last">
            ...
            minimal context with only the user object, not the full Auth0 client
            SDK
          </Sent>
          ...
          <Sent pos="last">
            ...
            because the middleware still needs <code>auth0.middleware()</code>{" "}
            and <code>auth0.getSession()</code> — that runs at the edge, not in
            the browser
          </Sent>
          ...
          <Sent pos="first">
            ...
            <em>shouldn&apos;t</em> be in a bundle rather than things that are
            too large. a 200kb package is fine if you actually need it. a 50kb
          ...
          <Sent pos="middle">
            ...
            being imported in a client component or shared module that the
            client also imports
          </Sent>
          <Sent pos="last">
      ...
      <section>
        ...
        <ul className="mt-2 space-y-2 text-muted">
          ...
          <li className="flex gap-2">
            <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-foreground/30" />
            <span>
              Opens an HTML treemap — each rectangle is a module, sized by its
              byte contribution
            </span>
          </li>
          <li className="flex gap-2">
        ...
        <ul className="mt-2 space-y-2 text-muted">
          ...
          <li className="flex gap-2">
            ...
            <span>
              ...
              is the middleware (intentionally large: it runs auth and CSP on
              every request).{" "}
              <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
                date-fns
              </code>{" "}
              at 24 modules is correct tree-shaking — only the calendar&apos;s
              named imports are included
            </span>
          </li>
        </ul>
      </section>
      ...
      <section>
        ...
        <ul className="mt-2 space-y-2 text-muted">
          ...
          <li className="flex gap-2">
            <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-foreground/30" />
            <span>
              Auth still works: middleware gates every route server-side before
              React runs; server components call{" "}
              <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
                auth0.getSession()
              </code>{" "}
      ...
      <section>
        ...
        <ul className="mt-2 space-y-2 text-muted">
          <li className="flex gap-2">
            <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-foreground/30" />
            <span>
              The analyzer is most useful for finding things that shouldn&apos;t
              be in a bundle, not just things that are too large
            </span>
          </li>
          <li className="flex gap-2">
            <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-foreground/30" />
            <span>
              Pattern to watch for: server-only libraries (crypto, JWT parsers,
              DB drivers) appearing in the client or edge bundle
            </span>
          </li>
          <li className="flex gap-2">
            <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-foreground/30" />
            <span>
              ...
              — zero results — confirmed the removal was safe before shipping
            </span>
          </li>
        </ul>
      </section>
