    ...
    >
      ...
      <WhatsNext
        ...
        couldImprove={[
          ...
          "The rung map is still a literal in the BFF, and the API should carry the field itself. What changed is the fallback: an unclassified flag now lands on the strictest rung rather than the loosest. `real ? admin : open` read as cautious and was not — the local seed holds four keys while the API serves more, so an upstream-only flag arrived with `real` undefined and fell straight to the rung that needs no session at all. Absent is not the same as false, and the guess has to go the safe way.",
