  ...
  pokemon_v2_pokemon(
    ...
    pokemon_v2_pokemonstats { base_stat pokemon_v2_stat { name } }
          ...
          <Sent pos="last">
            ...
            (avoid re-fetching objects you already have), optimistic mutations,
            or real-time subscriptions. I don&apos;t need any of that here, so
            the library would just be weight
          </Sent>
...
async function gql(query, variables, signal) {
  ...
  return data;
          ...
          <Sent pos="last">
            ...
            server component that calls it, wrapped in a <code>Suspense</code>{" "}
            boundary with a skeleton fallback. the skeleton streams immediately
            while the fetch resolves, then the real grid drops in. first paint
            shows actual Pokémon, no loading spinner
          </Sent>
          ...
          <Sent pos="first">
            ...
            filter key — <code>&quot;||&quot;</code>, empty search and no type —
            so <code>loading</code> is already <code>false</code> before the
          ...
          <Sent pos="last">
            ...
            and flips to false — after that, every filter change triggers a
            normal fetch. one-time skip, nothing special after
          </Sent>
          ...
          <Sent pos="last">
            ...
            <code>loadingMore</code> boolean and <code>isLoading</code> replaces
            the derived <code>loading = loadedKey !== filterKey</code>{" "}
            expression — both come straight from the hook, no manual state to
            keep in sync
          </Sent>
          ...
          <Sent pos="last">
            ...
            fetch with a 10-line wrapper is the right call here. Apollo would
            have been the cargo-cult choice
          </Sent>
    ...
    >
      <section>
        ...
        <p className="text-muted">
          The PokeAPI REST endpoint returns a massive JSON blob per Pokémon —
          game versions, form descriptions, encounter data — most of which a
          card view ignores. GraphQL lets you ask for exactly what the card
          needs. The PokeAPI v2 endpoint is powered by Hasura, which introspects
          the Postgres database and auto-generates the entire schema. Every
          table and relationship becomes queryable with filtering, sorting,
          aggregates, and nested joins — no resolvers written by hand.
        </p>
      </section>
      ...
      <section>
        ...
        <p className="text-muted">
          Apollo Client adds around 60kb gzipped to the client bundle. GraphQL
          is just HTTP — a POST request with a JSON body containing{" "}
          ...
          . Plain fetch handles that in about 10 lines. A client library earns
          its cost when you need a normalized cache, optimistic mutations, or
          real-time subscriptions. None of those are needed here, so the library
          would just be weight.
        </p>
      </section>
      ...
      <section>
        ...
        <p className="text-muted">
          ...
          variables object changes with user input. This means the network tab
          always shows the same query shape, which makes debugging easier and
          lets Hasura cache the parsed query on its side. Interpolation also
          opens the door to injection: someone passing{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            {"}) { id } query Anything {"}
          </code>{" "}
          as a search string could restructure the query entirely. Variables are
          always treated as scalars, never as query syntax.
        </p>
      </section>
      ...
      <section>
        ...
        <p className="text-muted">
          ...
          needed, since server code doesn&apos;t have CSP constraints. It passes{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            next: {"{"} revalidate: 3600 {"}"}
          </code>{" "}
          to the underlying fetch so repeated renders within an hour hit
          Next.js&apos;s data cache. The page wraps the server component in a
          Suspense boundary with a skeleton fallback — the skeleton streams
          immediately while the fetch resolves, then the real grid drops in.
        </p>
        <p className="mt-3 text-muted">
          ...
          cache as the first page. TanStack treats it as fresh for 30 seconds
          and skips the initial fetch. The seed only applies to the no-filter
          key — passing it unconditionally caused a bug where filtered queries
          got seeded with unfiltered server data and the 30-second stale time
          prevented the filter fetch from firing.
        </p>
      </section>
      ...
      <section>
        ...
        <p className="text-muted">
          ...
          — when either changes, TanStack cancels the in-flight request and
          fires a fresh fetch automatically.
        </p>
        <p className="mt-3 text-muted">
          The{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
          ...
          directly. Secondary benefit: the upstream URL stays out of the client
          bundle entirely.
        </p>
      </section>
      <WhatsNext
