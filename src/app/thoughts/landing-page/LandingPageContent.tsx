        ...
        <main className="mx-auto max-w-3xl px-4 py-10 sm:py-14">
          ...
          <div className="space-y-10 text-[15px] leading-relaxed text-foreground">
            ...
            <section>
              ...
              <p className="text-muted">
                The hero background is a WebGL water-plane shader with animated
                waves — a rolling B&W gradient that responds to mouse movement.{" "}
                <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
                  ShaderGradient
                </code>{" "}
                accepts{" "}
                ...
                as live React props. The cursor&apos;s normalized position
                within the element maps to angle ranges: X to azimuth 180–270, Y
                to polar 85–125.
              </p>
              <p className="mt-3 text-muted">
                Mouse updates are RAF-throttled — a{" "}
                <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
                  useRef
                </code>{" "}
                holds the target angles and a{" "}
                <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
                  requestAnimationFrame
                </code>{" "}
                callback writes them to state once per frame. The component is
                loaded with{" "}
                <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
                  next/dynamic
                </code>{" "}
                ...
                since WebGL needs a canvas and a window. The fallback is a plain{" "}
                <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
                  bg-black
                </code>{" "}
                div — LCP fires on the H1 text immediately while the gradient
                loads behind it. A{" "}
                <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
                  bg-black/50
                </code>{" "}
                scrim between the gradient and the text keeps the white text
                legible at all camera angles.
              </p>
            </section>
          </div>
        </main>
...
className="opacity-0 translate-y-8
  ...
  [animation-delay:100ms]"`}
              ...
              <Received pos="first">
                wait you said you wanted parallax on the hero blobs
              </Received>
              <Received pos="last">did you ever add that</Received>

              <Sent pos="first">
                went further. replaced the particle network with a
                ShaderGradient — a WebGL water-plane shader with animated waves.
                the hero background is now a rolling B&W gradient that responds
                to mouse movement
              </Sent>
              <Sent pos="last">
                move the mouse and the light source shifts with your cursor. the
                camera azimuth maps to horizontal movement, polar angle to
                vertical. feels like the surface is reacting to where you{"'"}re
                looking
              </Sent>
              ...
              <Sent pos="first">
                ...
                <code>cPolarAngle</code> as live React props — they control the
                camera orbit around the gradient surface. update those props and
                the library smoothly repositions the view
              </Sent>
              <Sent pos="middle">
                <code>onMouseMove</code> fires on the hero section. the cursor
                {"'"}s normalized position within the element maps to angle
                ranges: X to azimuth 180–270, Y to polar 85–125. so moving left
                to right sweeps the camera 90 degrees around the surface
              </Sent>
              <Sent pos="last">
                updates are RAF-throttled — a <code>useRef</code> holds the
                target angles and a <code>requestAnimationFrame</code> callback
                writes them to state once per frame.{" "}
                <code>smoothTime={"{0.18}"}</code> on the ShaderGradient handles
                the easing so it{"'"}s not jittery
              </Sent>
  ...
  if (rafRef.current === null) {
    rafRef.current = requestAnimationFrame(() => {
      ...
      rafRef.current = null;
              ...
              <Received>
                Three.js in a Next.js hero — how does that work without SSR
                exploding
              </Received>

              <Sent pos="first">
                <code>next/dynamic</code> with <code>ssr: false</code>.
                ShaderGradient needs a canvas and a window — neither exist on
                the server. the dynamic import keeps WebGL completely out of the
                server render
              </Sent>
              <Sent pos="last">
                the fallback is a plain <code>bg-black</code> div, so LCP fires
                on the H1 text immediately — the gradient loads in behind it
              ...
              <Received>
                what about the text being unreadable over the bright gradient
              </Received>

              <Sent pos="first">
                there{"'"}s a <code>bg-black/50</code> scrim between the
                gradient and the text — <code>absolute inset-0 z-[1]</code>. the
                gradient shows through at half opacity, the white text stays
                legible
              </Sent>
              <Sent pos="last">
                the text and button are <code>relative z-10</code>, above the
                scrim. the scrim sits above the gradient at <code>z-[1]</code>.
                pointer events on the canvas are disabled so clicks still reach
                the UI elements
              </Sent>
              ...
              <Sent pos="first">
                could widen the angle sweep for more dramatic movement. right
                now it{"'"}s 90 degrees of azimuth — you could go 180 for a full
                half-orbit across one mouse swipe
              </Sent>
              <Sent pos="last">
                <code>prefers-reduced-motion</code> check would be good too —
                lock the camera to the default angles and skip the mouse handler
                entirely when the user has reduced motion on. the gradient
                animation itself should also pause in that case
              </Sent>
