...
const HREF = "/thoughts/mac-menu-bar";

describe("macOS Menu Bar write-up", () => {
  ...
  it("renders the heading and the sections that document the work", () => {
    render(<MacMenuBarContent />);
    expect(
      screen.getByRole("heading", { level: 1, name: "macOS Menu Bar" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: /derive the menu from state/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: /keyboard and screen-reader/i }),
    ).toBeInTheDocument();
  });
});
