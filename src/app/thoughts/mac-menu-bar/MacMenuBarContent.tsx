    ...
    <ThoughtLayout
      ...
      intro={
        <>
          ...
          Help labels along the top was dead paint. This is the pass that turned
          it into a real macOS menu system: a signal-driven service that derives
          every menu from the live window and dock state, dropdowns that run
          actual actions, and keyboard access that holds up to an axe scan with a
          menu open.
        </>
      }
    >
      ...
      <section>
        <h2 className="mb-3 text-lg font-bold">A service, not component state</h2>
        <p className="text-muted">
          The menu model went into a new{" "}
          <C>MenuBarService</C> (<C>providedIn: &quot;root&quot;</C>), not into
          the component. Two reasons. First, the model is derived data that
          several things may want to read, so it belongs where the other derived
          desktop state lives, next to the window and dock services it depends
          on. Second, it keeps the <C>MenuBar</C> component thin: the component
          renders the model and reports clicks, and the service decides what a
          click <em className="text-foreground/80">means</em>.
        </p>
        <p className="mt-3 text-muted">
      ...
      <section>
        ...
        <p className="text-muted">
          A menu bar is a classic a11y trap: it&rsquo;s trivial to build one that
          only works with a mouse. This one was built to the menu pattern from
          the start. Top-level menus are real <C>&lt;button&gt;</C> elements with{" "}
          <C>aria-haspopup</C> and <C>aria-expanded</C>; open dropdowns are{" "}
          <C>role=&quot;menu&quot;</C> with <C>role=&quot;menuitem&quot;</C>{" "}
          children, each a native button so it&rsquo;s focusable and operable by
          keyboard, with the shortcut shown as a hint.
        </p>
        <ul className="mt-3 space-y-2 text-muted">
          <li>
            ...
            to the menu that opened it, so a keyboard user is never dumped at the
            top of the document.
          </li>
          <li>
            <span className="font-semibold text-foreground">
            ...
            , and hovering a sibling top-level menu while one is open switches to
            it &mdash; the way the real menu bar behaves.
          </li>
          <li>
            <span className="font-semibold text-foreground">
              Click-outside closes
            </span>{" "}
            without stranding focus, and the translucent dropdown styling matches
            the desktop&rsquo;s existing visual language rather than inventing a
            new one.
          </li>
        </ul>
      </section>
      ...
      <section>
        <h2 className="mb-3 text-lg font-bold">What shipped, and how it&rsquo;s verified</h2>
        <p className="text-muted">
          The suite went from 229 to 252 tests: 14 new{" "}
          <C>MenuBarService</C> unit tests covering the derived model and every
          action, new interaction tests for the <C>MenuBar</C> component, and an
          added axe scan of the menu bar{" "}
          <em className="text-foreground/80">with a menu open</em> &mdash; the
          state most menu-bar a11y tests skip. All 252 pass, ESLint is clean over{" "}
          <C>src/</C>, <C>tsc --noEmit</C> is clean, and the production build
          succeeds. WCAG 2.1 AA holds with the menu expanded, not just closed.
        </p>
      </section>
      ...
      <section>
        ...
        <p className="text-muted">
          ...
          chrome, that warning is the first place a real budget problem will show
          up.
        </p>
      </section>
    </ThoughtLayout>
