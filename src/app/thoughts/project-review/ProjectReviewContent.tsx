    ...
    >
      ...
      <section>
        ...
        <p className="text-muted">
          Four lenses, all reproducible: an axe accessibility scan (WCAG 2.1 AA
          + best-practice) across every public and authenticated route; a census
          of lines-of-code and duplication by directory (<C>wc -l</C> +{" "}
          <C>grep</C> for repeated class strings and copied scaffolds); a
          production build for bundle weight; and a test-file census to find
          untested surface. The numbers below come from those, not vibes.
        </p>
        <p className="mt-3 text-muted">
          Headline shape:{" "}
          <span className="font-semibold text-foreground">
            ~96,300 lines of <C>src</C>, 80 page routes, 46 API routes, 94 test
            files
          </span>
      ...
      <section>
        ...
        <ul className="space-y-2 text-muted">
          <li>
            ...
            <C>?version=v1|v2|v3</C> resolved through a <C>VERSIONS</C> registry
            in <C>page.tsx</C>, with retired versions behind <C>next/dynamic</C>{" "}
            so their deps stay off the default path. It&rsquo;s the cleanest
            idea in the repo &mdash; three full landing eras coexist with zero
            conditional soup.
          </li>
          <li>
            <span className="font-semibold text-foreground">
            ...
            <C>getBackendAuth</C>, <C>buildHeaders</C>) rather than copied per
            route. Tokens never reach the client.
          </li>
          <li>
            <span className="font-semibold text-foreground">
              A real data layer.
          ...
          <li>
            ...
            14 public routes plus the 3 authenticated ones are scanned in CI at
            AA + best-practice and are clean. That bar didn&rsquo;t exist a week
            ago (see the measured gains at the end).
          </li>
        </ul>
      </section>
        ...
        <div className="space-y-7">
          ...
          >
            <p>
              ...
              . Each write-up is a bespoke component: the same <C>PageHeader</C>{" "}
              + &ldquo;Dev notes&rdquo; eyebrow + <C>main</C> scaffold repeated
              35 times, with individual files running 1,000&ndash;2,000 lines
              (the landing-page write-up alone is 1,991).
            </p>
            <p>
              <Tag kind="con" /> Cross-cutting changes are 35&times; work. The
              a11y sweep proved it: moving the <C>h1</C> into a landmark, or
              making the before/after code block keyboard-scrollable, is one
              edit in a shared layout and N edits here. Prose lives in JSX, so
              it&rsquo;s awkward to write and diff.
            </p>
            <p>
            ...
            <p>
              ...
              for the interactive bits) plus a shared <C>ThoughtLayout</C> would
              keep the freedom while deleting the scaffold. Rough order: the
              repeated layout + eyebrow + code-bubble boilerplate is a few
          ...
          >
            <p>
              Four fantasy pages carry a hand-rolled <C>&lt;section&gt;</C>{" "}
              filter bar with the identical styled <C>&lt;select&gt;</C> (
            ...
            <p>
              <Tag kind="con" /> Every one drifted slightly, and each needed its
              own <C>aria-label</C> + landmark fix in the accessibility pass. A
              bug or a restyle is N edits.
            </p>
            <p>
              <Tag kind="gain" /> One <C>&lt;FilterBar&gt;</C> +{" "}
              <C>&lt;LabelledSelect&gt;</C> collapses 5 files of boilerplate to
              a handful of props and makes the next a11y/UX fix a one-file
              change.
            </p>
          </Finding>
          ...
          >
            <p>
              <C>v3/graph/simulation.ts</C> (278 lines) and <C>graphData.ts</C>/
              <C>buildLayeredLayout</C> are pure functions &mdash; deterministic
              given a seed &mdash; yet there is not a single unit test for them.
              Meanwhile the giant learn-content pages{" "}
              <em className="text-foreground/80">do</em> have a parameterised
              suite.
            </p>
            <p>
            ...
            <p>
              <Tag kind="gain" /> A dozen cheap assertions (settling converges,
              no two nodes overlap after N ticks, the layered layout assigns
              every node a column) would lock the behaviour for near-zero cost,
              because the functions take plain data in and out.
            </p>
          </Finding>
          ...
          >
            <p>
              The learn steppers share a shape but not an implementation, so the
              play/advance off-by-one had to be fixed ten times. Same pattern
              with the fantasy selects (E2) and the thoughts scaffold (E1).
            </p>
            <p>
        ...
        <div className="space-y-7">
          ...
          >
            ...
            <p>
              ...
              serve wildly different needs, and no single feature is deep enough
              to justify the others. That&rsquo;s fine here &mdash; but
              it&rsquo;s the reason &ldquo;good overall system design&rdquo; is
              the wrong yardstick. The right one is &ldquo;does the shell stay
              thin and consistent across unrelated features,&rdquo; and mostly
              it does.
            </p>
          </Finding>
          ...
          >
            ...
            <p>
              ...
              &rarr; typed 502, consistent logging) applied to every route makes
              the proxy layer uniformly resilient and deletes the copied
              try/catch blocks.
            </p>
          </Finding>
          ...
          >
            <p>
              ...
              method. The auth/header plumbing is already shared (good), but the
              route bodies still repeat the fetch-map-return dance.
            </p>
            <p>
      ...
      <section>
        ...
        <div className="space-y-7">
          ...
          >
            <p>
              v3 is{" "}
              <span className="font-semibold text-foreground">1,503 lines</span>{" "}
              of bespoke code: a force-directed simulation (repulsion, springs,
              gravity, collision, label-aware spacing), a fit-to-viewport
              renderer, a second flat layout engine, a mobile fallback, and GSAP
              &mdash; all for the landing page.
            </p>
            <p>
            ...
            <p>
              ...
              visitors skim once. A library (react-flow, d3-force) or a simpler
              animated static layout would land ~80% of the effect for a
              fraction of the code and maintenance. The flat view + mobile path
              roughly double the footprint for a fallback.
            </p>
            <p>
              ...
              landing carries product-grade engineering, and the guardrail is to
              keep it walled off (which it is) and tested (which it isn&rsquo;t,
              see E3).
            </p>
          </Finding>
          ...
          >
            <p>
              Treating every write-up and lesson as a hand-built React component
              &mdash; rather than content fed through a pipeline &mdash; is an
              architectural stance. It optimises for &ldquo;drop any interactive
              demo mid-sentence&rdquo; at the cost of every other content
              operation (authoring, diffing, translating, shared layout, bulk
              fixes).
            </p>
            <p>
              <Tag kind="gain" /> MDX (or a headless CMS for the pure prose)
              keeps the interactive escape hatch while making the other 95% of
              each page cheap. This is the highest-leverage refactor in the repo
              by line count.
            </p>
          </Finding>
          ...
          >
            <p>
              ...
              ) and its own component folder &mdash; genuinely well-architected,
              arguably the best-engineered feature. That is also the point: it
              carries production-app depth for a demo.
            </p>
            <p>
            ...
            <p>
              <Tag kind="con" /> The investment is disproportionate to a feature
              nobody depends on; if effort is the scarce resource, that depth is
              &ldquo;spent&rdquo; where E1/E3 would have paid back more.
            </p>
          </Finding>
        </div>
      </section>
      ...
      <section>
        ...
        <p className="mb-4 text-muted">
          Each feature works; this is the single most valuable improvement for
          each, not a teardown.
        </p>
      ...
      <section>
        ...
        <p className="mb-3 text-muted">
          The accessibility work that preceded this review is the one place with
          ...
          <table className="w-full text-left text-[13px]">
            ...
            <tbody className="text-muted">
              ...
              <tr className="border-b border-border/50">
                ...
                <td className="py-2 text-foreground">AA + best-practice</td>
              </tr>
              <tr className="border-b border-border/50">
        ...
        <p className="mt-4 text-muted">
          The rest of this review is a map, not a receipt &mdash; the gains in
          E1&ndash;A3 are estimated from line counts and duplication factors,
          and become real numbers only once the refactors land. That&rsquo;s the
          honest state: the accessibility bar is measured and done; the
          structural debt is identified, quantified where possible, and
          prioritised.
        </p>
      </section>
      <WhatsNext
