          ...
          <div className="space-y-10 text-[15px] leading-relaxed text-foreground">
            ...
            <section>
              ...
              <p className="text-muted">
                The hub page is a server component — it calls <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">await auth0.getSession()</code> to get the user session. Server components can&apos;t use <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">useState</code> or handle user input, so the search input was just decoration. Making it interactive required something to become a client component.
              </p>
            </section>
            ...
            <section>
              ...
              <p className="text-muted">
                Converting the whole page to a client component would mean losing server-side auth — <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">auth0.getSession()</code> only works in server components, where it can read cookies from the request. Instead, only the search bar and thread list were extracted into a <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">ThreadList</code> client component.
              </p>
              <p className="mt-3 text-muted">
                The page itself stays server-rendered: it fetches the session, reads the user name and email, and renders the layout. It passes the <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">threads</code> array down as a prop. The client component handles interactive filtering. The principle: push client boundaries as low as possible — only the leaf that needs interactivity gets <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">&quot;use client&quot;</code>.
              </p>
            </section>
            ...
            <section>
              ...
              <p className="text-muted">
                Simple <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">.filter()</code> on every keystroke — case-insensitive <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">.includes()</code> check against three fields: name, href, and preview. With 4 threads this is instant. No debouncing, no fuzzy matching, no external library needed.
              </p>
            </section>
            ...
            <section>
              ...
              <p className="text-muted">
                Server components are the default in Next.js for a reason — they don&apos;t ship JS to the browser, they can access server-only APIs directly, and they render on first paint without hydration delay. Making the whole page a client component would have been simpler to write, but it&apos;s a bad pattern to establish. The right amount of complexity here is the minimum needed: <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">.filter()</code> and <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">.includes()</code> over 4 items. No more.
              </p>
            </section>
