...
export default function SearchBarContent() {
  return (
    <ThoughtLayout
      ...
      intro={
        <>
          Making a search input interactive without converting the whole page to
          a client component — pushing the client boundary as low as possible.
        </>
      }
      chat={
        <ChatThread>
          ...
          <Sent pos="middle">
            ...
            reads cookies from the request. move that to the client and it
            breaks
          </Sent>
          <Sent pos="last">
            instead i <strong>extracted</strong> just the search bar + thread
            list into a <code>ThreadList</code> client component
          </Sent>
          ...
          <Sent pos="first">
            <strong>server component</strong> keeps: auth session, user info,
            top bar, bottom bar with logout link, thread data definition
          </Sent>
          <Sent pos="last">
...
export default function ThreadList({ threads }) {
  ...
  return <>{/* search + list */}</>;
          ...
          <Sent pos="last">
            ...
            only the search input and its filtered list hydrate on the client
          </Sent>
          ...
          <Sent pos="first">
            ...
            serialized across the boundary as props. for 4 items that
            {"'"}s nothing
          </Sent>
          <Sent pos="middle">
            if we had hundreds of threads, we{"'"}d want server-side filtering
            instead — send the query as a search param and filter before
            rendering
          </Sent>
          <Sent pos="last">
            also the CSS Module import is now shared between the server page and
            the client component. works fine but it{"'"}s something to be aware
            of — both need access to the same class names
          </Sent>
    ...
    >
      <section>
        ...
        <p className="text-muted">
          ...
          or handle user input, so the search input was just decoration. Making
      ...
      <section>
        ...
        <p className="text-muted">
          Converting the whole page to a client component would mean losing
          server-side auth —{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            auth0.getSession()
          </code>{" "}
          only works in server components, where it can read cookies from the
          request. Instead, only the search bar and thread list were extracted
          into a{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            ThreadList
          </code>{" "}
        ...
        <p className="mt-3 text-muted">
          The page itself stays server-rendered: it fetches the session, reads
          the user name and email, and renders the layout. It passes the{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            threads
          </code>{" "}
          array down as a prop. The client component handles interactive
          filtering. The principle: push client boundaries as low as possible —
          only the leaf that needs interactivity gets{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            &quot;use client&quot;
          </code>
          .
      ...
      <section>
        ...
        <p className="text-muted">
          ...
          check against three fields: name, href, and preview. With 4 threads
          this is instant. No debouncing, no fuzzy matching, no external library
          needed.
        </p>
      </section>
      ...
      <section>
        ...
        <p className="text-muted">
          ...
          don&apos;t ship JS to the browser, they can access server-only APIs
          directly, and they render on first paint without hydration delay.
          Making the whole page a client component would have been simpler to
          write, but it&apos;s a bad pattern to establish. The right amount of
          complexity here is the minimum needed:{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            .filter()
          </code>{" "}
          and{" "}
