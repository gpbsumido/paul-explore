          ...
          >
            <div className={styles.chat}>
              ...
              <Sent pos="middle">
                ...
                — each request would generate a random nonce and any script
                without that nonce attribute would be blocked
              </Sent>
              <Sent pos="last">
                locally it was fine because the dev server always re-renders
                everything. in production the landing page became fully static
                after moving the auth redirect to middleware, and Vercel served
                the CSP from the CDN edge
              </Sent>
              ...
              <Sent pos="first">
                Next.js App Router inlines RSC payload scripts directly into the
                HTML — they look like <code>self.__next_f.push([...])</code>.
                these are what hydrate the React tree on the client
              </Sent>
              <Sent pos="middle">
                the problem is they have no nonce attribute. Next.js just
                inlines them as-is. and <code>&apos;strict-dynamic&apos;</code>{" "}
                explicitly ignores <code>&apos;self&apos;</code>, so even
                loading scripts from your own origin stops working unless they
                carry the nonce
              </Sent>
              <Sent pos="last">
                when the page was dynamic, server-rendered responses may have
                behaved differently. once it went static, Vercel&apos;s edge
                middleware applied the CSP header to every CDN-cached response —
              ...
              <Sent pos="last">
                ...
                DevTools → Console → CSP violation errors pointing at the inline
                RSC scripts
              </Sent>
              ...
              <Sent pos="first">
                ...
                . allows scripts from the same origin and allows inline scripts.
                that covers both the RSC payloads and the Next.js chunks loaded
                from /_next/static/
              </Sent>
              <Sent pos="last">
                also had to add <code>https://vitals.vercel-insights.com</code>{" "}
                to <code>connect-src</code> — Speed Insights was silently
                failing to send its beacons because{" "}
                <code>connect-src &apos;self&apos;</code> blocked the request to
              ...
              <Sent pos="middle">
                ...
                <code>&lt;script&gt;</code> tag into the HTML response. CSP
                blocks that script from running even if it makes it into the
                markup
              </Sent>
              <Sent pos="last">
                ...
                <code>dangerouslySetInnerHTML</code> — and there&apos;s none of
              ...
              <Received>
                what about the calendar? users put data in there
              </Received>

              <Sent pos="first">
                yeah I checked. calendar events go through the same JSX
                rendering pipeline — event titles, descriptions, card names, all
                rendered as text nodes via React. no raw HTML injection path
              </Sent>
              <Sent pos="middle">
                if I were ever rendering user content as HTML (like a rich text
                field), that would change things. you&apos;d want either a
                strict sanitizer like DOMPurify before passing to
                dangerouslySetInnerHTML, or a nonce — which brings us back to
                the tradeoff
              </Sent>
              <Sent pos="last">
              ...
              <Sent pos="first">
                to do nonce-based CSP correctly in Next.js you have to make the
                root layout async and read the nonce from request headers inside
                it, so Next.js can stamp it onto the RSC inline scripts
              </Sent>
              <Sent pos="middle">
                but reading headers() in any server component opts that route
                out of static generation. it becomes dynamic — re-rendered on
                every request. the root layout wraps every page, so every page
                goes dynamic
              </Sent>
              <Sent pos="last">
                I actually tried this. ran a build, and every page flipped from
                ○ Static to ƒ Dynamic. TTFB, LCP, FCP — all the vitals I&apos;ve
                been optimizing — take a direct hit. it&apos;s not worth it here
              </Sent>
              ...
              <Sent pos="first">
                for this site, yes. it&apos;s the standard CSP approach for
                Next.js apps with static pages — it&apos;s what the Next.js docs
                recommend for exactly this reason
              </Sent>
              <Sent pos="middle">
                the remaining directives do meaningful work:{" "}
                <code>default-src &apos;self&apos;</code> blocks loading
                resources from unknown domains,{" "}
                <code>frame-ancestors &apos;none&apos;</code> blocks
                clickjacking, <code>object-src &apos;none&apos;</code> blocks
                Flash and plugins, <code>base-uri &apos;self&apos;</code> blocks
                base tag injection
              </Sent>
              <Sent pos="last">
                and connect-src is locked down so JS can only make requests to
                known endpoints — same origin, plus the specific external
                services that need it (Speed Insights, TCGdex, GitHub raw for
                Pokémon sprites)
              </Sent>
              ...
              <Sent pos="first">
                ...
                <code>&apos;unsafe-inline&apos;</code> the CSP part was simpler,
                but the bigger issue was <code>auth0.middleware()</code> itself.
                it makes a network call to Auth0 on every single request — not
                just auth routes, every page
              </Sent>
              <Sent pos="middle">
                so the landing page, TCG browser, all of it — every visit was
                paying that latency cost. it showed up in TTFB data immediately.
                the whole middleware got pulled to stop the bleeding
              </Sent>
              <Sent pos="last">
                the protected page&apos;s auth was handled differently during
                that period — <code>page.tsx</code> was calling{" "}
                <code>auth0.getSession()</code> as a redirect guard, which just
                reads a cookie with no network call. but that made the page
                dynamic, which is its own TTFB problem
              </Sent>
              ...
              <Sent pos="first">
                ...
                <code>middleware.ts</code> — you can only have one. we were
                running both, which broke the build, so everything got
                consolidated into <code>proxy.ts</code> with a broad matcher
              </Sent>
              <Sent pos="middle">
                the broad matcher means the proxy runs on every request. but the
              ...
              <Sent pos="middle">
                ...
                cookie decrypt. if there&apos;s no session it redirects to login
                and <code>auth0.middleware</code> never runs. if there is a
                session, <code>auth0.middleware</code> runs after to handle
                rolling session refresh
              </Sent>
              <Sent pos="last">
                the root <code>/</code> route is a dynamic server component now
                — it calls <code>auth0.getSession()</code> directly and renders
                either the landing page or the hub. no middleware redirect
                needed. the session check is a local cookie decrypt, so
                there&apos;s no network cost and the dynamic render penalty is
                negligible
              </Sent>
            </div>
          </div>
