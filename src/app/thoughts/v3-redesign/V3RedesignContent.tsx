    ...
    >
      <section>
        ...
        <p className="text-muted">
          ...
          hand-authored &mdash; add a feature or a thought and it just appears,
          wired up.
        </p>
      </section>

      <section>
        <h2 className="mb-3 text-lg font-bold">Two views, one data model</h2>
        <p className="text-muted">
          The same nodes and edges render two ways. The{" "}
          <span className="font-semibold text-foreground">force</span> view is
          the playground: draggable nodes, a physics settle, hover to highlight
          a node and its neighbours. The{" "}
          ...
          column, its items stacked below) that fits on a screen and is easy to
          scan. A switch in the header flips between them; the data layer (
          <code className="rounded bg-surface px-1 py-0.5 font-mono text-[13px] text-foreground">
            graphData.ts
          </code>
      ...
      <section>
        ...
        <p className="mt-3 text-muted">
          ...
          coordinate space, and the renderer{" "}
          <em className="text-foreground/80">fits</em> that space to the
          viewport every frame. So &ldquo;use the available space&rdquo; and
          &ldquo;lay out the graph&rdquo; are decoupled &mdash; the physics
          never has to know the screen size, and resizing just re-fits.
        </p>
      </section>
      ...
      <section>
        ...
        <ul className="mt-2 space-y-2 text-muted">
          ...
          <li>
            ...
            The force and radius buffers are allocated once and reused each tick
            instead of a fresh{" "}
            <code className="rounded bg-surface px-1 py-0.5 font-mono text-[13px] text-foreground">
              Float64Array
            </code>{" "}
      ...
      <section>
        ...
        <ul className="mt-2 space-y-2 text-muted">
          ...
          <li>
            <span className="font-semibold text-foreground">
              The graph slides out from under the cursor.
            </span>{" "}
            When you hover a node its neighbours push away, which grows the
            bounding box, which re-fits the whole graph &mdash; moving the node
            you were pointing at. Fix: freeze the fit while dragging or
            focusing, and pin the focused node.
          </li>
          <li>
            <span className="font-semibold text-foreground">
              Labels overlap even when dots don&rsquo;t.
            </span>{" "}
          ...
          <li>
            <span className="font-semibold text-foreground">
              Push first, or highlight first?
            </span>{" "}
            Highlighting a node and immediately shoving its neighbours felt
            jumpy, so the push waits about half a second &mdash; the selection
            locks in, then the space opens up.
          </li>
        </ul>
      </section>
      ...
      <section>
        ...
        <ul className="mt-2 space-y-2 text-muted">
          <li>
            ...
            order, have accessible names, and open on Enter &mdash; dragging is
            a pointer-only enhancement on top. Every node has a visible focus
            ring.
          </li>
          <li>
            Proper landmarks (
            ...
            ; decorative layers (background, legend, sparks) are{" "}
            <code className="rounded bg-surface px-1 py-0.5 font-mono text-[13px] text-foreground">
              aria-hidden
            </code>
            .
          ...
          <li>
            The landing passes the automated axe scan at WCAG 2.1 AA with zero
            violations, and axe&rsquo;s stricter
            <em className="text-foreground/80"> best-practice</em> rules too.
            Flipping best-practice on globally flagged a few older routes that
            predate landmark discipline, so that became its own follow-up sweep
            &mdash; the shared test now enforces best-practice, and the routes
            it caught (a couple of{" "}
            <code className="rounded bg-surface px-1 py-0.5 font-mono text-[13px] text-foreground">
              /tcg
            </code>{" "}
            pages,{" "}
