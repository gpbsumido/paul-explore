...
export default function WebVitalsContent() {
  return (
    <ThoughtLayout
      ...
      intro={
        <>
          A self-hosted vitals pipeline that collects real-user data from every
          page and aggregates it into P75 scores — built because owning the data
          matters more than reading a third-party dashboard.
        </>
      }
      chat={
        <ChatThread>
          ...
          <Sent pos="first">
            ...
            environment on a simulated device. the dashboard shows data from
            real users on real connections
          </Sent>
          <Sent pos="middle">
            ...
            dashboard at <code>/vitals</code> aggregates that into P75 scores
            per metric and per page
          </Sent>
          <Sent pos="last">
            so if somebody loads the calendar on a slow 4G connection in another
            country, that shows up in the numbers. Lighthouse wouldn&apos;t
            catch that
          </Sent>
          ...
          <Sent pos="first">
            LCP — Largest Contentful Paint. how long until the biggest element
            on the page is visible. good is under 2.5s. this is the main
            perceived load speed metric
          </Sent>
          <Sent pos="middle">
            FCP — First Contentful Paint. time until any content shows up at
            all. good is under 1.8s
          </Sent>
          <Sent pos="middle">
            INP — Interaction to Next Paint. measures how quickly the page
            responds to clicks, taps, and key presses. replaced FID in 2024 as
            the official interactivity metric. good is under 200ms
          </Sent>
          <Sent pos="middle">
            CLS — Cumulative Layout Shift. a score for how much content jumps
            around while the page loads. score of 0 is perfect, under 0.1 is
            good. the one that makes you click the wrong button because the page
            shifted
          </Sent>
          <Sent pos="last">
            TTFB — Time to First Byte. how long the browser waits for the server
            to start sending a response. good is under 800ms. a high TTFB
            usually means something slow is happening server-side before any
            HTML arrives
          </Sent>
          ...
          <Received pos="first">
            wait, Vercel already has Speed Insights
          </Received>
          <Received pos="last">why build your own?</Received>

          <Sent pos="first">
            a few reasons. Vercel Speed Insights is still in the app — it feeds
            into their dashboard and that&apos;s useful. but the data lives
            there, in their UI, not mine
          </Sent>
          <Sent pos="middle">
            if I want to display vitals inside the app itself — like on the
            protected hub — I need to own the data. self-hosted means I can
            query it any way I want
          </Sent>
          <Sent pos="last">
          ...
          <Sent pos="first">
            ...
            the right time and fires a callback with the name, value, and a
            rating
          </Sent>
          <Sent pos="middle">
            <code>WebVitalsReporter</code> is a client component in the root
            layout. it registers all five collectors once on mount and sends
            each metric to <code>/api/vitals</code> when it fires. that&apos;s a
            Next.js route that validates the shape and forwards it to the
            Express backend
          </Sent>
          <Sent pos="last">
  ...
  → INSERT INTO web_vitals`}
          ...
          <Sent pos="last">
            the one catch: sendBeacon sends as <code>text/plain</code> by
            default, which the Express JSON parser won&apos;t read. you need to
            wrap the body in a <code>Blob</code> with an explicit{" "}
  ...
  new Blob([JSON.stringify(payload)], {
    type: "application/json",
          ...
          <Sent pos="last">
            ...
            immediately navigates away, INP and CLS haven&apos;t fired yet —
            they fire on the page hide event, which is exactly when a regular
            fetch would get killed
          </Sent>
          ...
          <Sent pos="last">
            5 samples is a low bar but it&apos;s enough to filter out
            single-visit artifacts. with real traffic you&apos;d want more —
            50+, 100+ — but for a portfolio site 5 is a reasonable start
          </Sent>
          ...
          <Sent pos="first">
            ...
            component — it calls <code>auth0.getAccessToken()</code> to get a
            JWT, then fetches <code>/api/vitals/summary</code> and{" "}
            <code>/api/vitals/by-page</code> from the Express backend in
            parallel with <code>Promise.all</code>
          </Sent>
          <Sent pos="middle">
            the token never touches the browser. the backend verifies it with
            the same <code>checkJwt</code> middleware as every other protected
            route
          </Sent>
          <Sent pos="last">
            the summary and by-page fetches use{" "}
            <code>next: {`{ revalidate: 60 }`}</code> instead of{" "}
            <code>no-store</code>. aggregate vitals data doesn&apos;t change on
            every request, it changes when new rows come in. 60 seconds is fresh
            enough and saves a backend round trip for anyone who refreshes
  ...
  fetch(\`\${API_URL}/api/vitals/by-page\`, {
    ...
    next: { revalidate: 60 },
          ...
          <Sent pos="first">
            ...
            the bundle at build time. the dashboard nav has a dropdown that
            filters all aggregates to &quot;from version X onwards&quot; so you
            can see whether a specific deploy actually moved the numbers
          </Sent>
          <Sent pos="middle">
            there&apos;s also a trend chart now using unovis — one sparkline per
            metric showing P75 across the last 5 versions. line color follows
            the Good/Poor thresholds so you can tell at a glance if things are
            going the right direction
          </Sent>
          <Sent pos="last">
            still want: device and connection breakdown (the{" "}
            <code>web-vitals</code> package gives <code>navigationType</code>{" "}
            but not device category or effective connection type — you&apos;d
            need the Network Information API for that), and alert thresholds so
            a bad P75 actually notifies you instead of waiting for someone to
            open the dashboard
          </Sent>
          ...
          <Sent pos="first">
            ...
            real users on real devices with real network conditions are the
            answer
          </Sent>
          <Sent pos="middle">
            and that I can instrument a full observability pipeline —
            collection, transport, storage, aggregation, display — without
            reaching for a third-party service for every piece. each part of
            this is standard browser and backend technology wired together
          </Sent>
          <Sent pos="last">
            the data pipeline here is the same shape as any event analytics
            system. knowing how to build it for web vitals means you can adapt
            it for custom business events, error tracking, or feature usage
            metrics with minimal changes
          </Sent>
          ...
          <Sent>
            ...
            actually see whether the ISR caching, streaming SSR, and
            next/dynamic lazy loading are making a measurable difference to real
            users, not just green numbers on a report. the version filter makes
            that concrete: pick the version where streaming SSR shipped and see
            if LCP dropped
          </Sent>
          ...
          <Received>
            you mentioned INP is under 200ms. how do you keep that low
          </Received>

          <Sent pos="first">
            two things. the obvious one: don&apos;t do expensive work on
            interaction. but the subtler one is CSS —{" "}
            <code>transition-all</code> tells the browser to watch every CSS
            property for changes on every animation frame. even if only opacity
            changes, the browser still has to check border, padding, font-size,
          ...
          <Sent pos="last">
            ...
            <code>startTransition</code> marks it as non-urgent — React
            processes any pending input events first before repainting all those
            cards. same pattern in the version selector:{" "}
            <code>router.push()</code> in a transition so selecting a different
            version doesn&apos;t block whatever the user was doing
          </Sent>
          ...
          <Received>
            what about /protected — that page had bad TTFB too
          </Received>

          <Sent pos="first">
            same root cause. the page component was calling{" "}
            <code>auth0.getSession()</code> to get the user&apos;s name and
            email to pass down to <code>FeatureHub</code>. that one call makes
            Next.js treat the route as dynamic, so every visit hit a cold
            serverless function with a 1.5–3s startup penalty
          </Sent>
          <Sent pos="middle">
            the fix: make <code>page.tsx</code> a plain sync component with no
            auth calls. Next.js can now statically pre-render it at build time
            and Vercel serves the HTML from CDN edge — TTFB drops from around
            2.1s to roughly 50ms
          </Sent>
          <Sent pos="last">
            user info (name, email) moves to a small <code>GET /api/me</code>{" "}
            BFF route. <code>FeatureHub</code> fetches it on mount and shows
            skeleton bones in the header while in-flight. the cookie decrypt
            server-side is fast — even with network round-trip the user details
            fill in well after first paint, so FCP and LCP are unaffected
          </Sent>
  ...
  fetch("/api/me")
    ...
    .then(({ name, email }) => {
      ...
      setUserEmail(email ?? undefined);
          ...
          <Sent pos="first">
            the entrance animations. the hub page uses a <code>reveal()</code>{" "}
            helper that starts elements at <code>opacity-0 translate-y-8</code>{" "}
            and fades them in after hydration. the H1 heading was wrapped in it,
            same as the cards
          </Sent>
          <Sent pos="middle">
            the problem: browsers exclude <code>opacity: 0</code> elements from
            LCP consideration entirely. the H1 is the biggest text element on
            the page, but it doesn&apos;t count until it becomes visible. so LCP
            was measured after hydration, plus a 700ms CSS transition, plus
            whatever bundle parse time took. easily 2.5s+
          </Sent>
          <Sent pos="last">
          ...
          <Sent pos="first">
            yeah, that one was a fetch waterfall I introduced. the page needed
            to default to the latest version, so it fetched versions and
            byVersion first, then started the main vitals fetch once those
            resolved. two sequential backend round trips before any HTML could
            go out
          </Sent>
          <Sent pos="middle">
          ...
          <Sent pos="last">
            ...
            filters correctly. one backend round trip saved on every page load
            is worth that
          </Sent>
...
const selectedVersion = urlVersion ?? versions[0];`}
          ...
          <Sent pos="first">
            it was a flat dropdown of every patch version. fine with 10 entries,
            unusable once you have 30+. so I grouped them into tiers
          </Sent>
          <Sent pos="middle">
            the top two options are &quot;Current Major&quot; (all data in the
            major version, the default) and &quot;Current Minor&quot; (all
            patches in the latest minor). below that, the last 3 minor versions
            show every individual patch in optgroups. anything older collapses
            into one entry per minor with aggregated data
          </Sent>
          <Sent pos="middle">
            the URL encodes the filter mode via prefix — <code>major:0</code>{" "}
            for all 0.x.y, <code>minor:0.12</code> for all 0.12.x, or just{" "}
            <code>0.11.3</code> for an exact match. the backend has a{" "}
            <code>buildVersionConditions</code> helper that translates the mode
            param into the right SQL WHERE clause
          </Sent>
          <Sent pos="last">
          ...
          <Sent pos="first">
            ...
            shows a skeleton grid while hydrating, then swaps in the real
            charts. the skeleton used <code>h-20</code> (80px) for each chart
            area
          </Sent>
          <Sent pos="middle">
            the real chart uses{" "}
            <code>VisXYContainer height=&#123;80&#125;</code>
            for the plot area, but also includes{" "}
            <code>VisAxis type=&quot;x&quot;</code> which renders version tick
            labels below that 80px boundary. actual rendered height is around
            100px. all five metric charts swapping simultaneously added about
            20px of shift each
          </Sent>
          <Sent pos="last">
            fix: extract <code>CHART_AREA_HEIGHT = 80</code> and{" "}
            <code>CHART_CONTAINER_HEIGHT = CHART_AREA_HEIGHT + 20</code> at the
            top of the file. both the skeleton div and the real chart wrapper
            use <code>CHART_CONTAINER_HEIGHT</code> so they reserve the same
            space. future height changes only need to happen in one place
          </Sent>
    ...
    >
      ...
      <section>
        ...
        <p className="text-muted">
          ...
          dashboard and that&apos;s useful. But the data lives there, in their
          UI, not mine. Self-hosted means the data can be queried any way
          needed, displayed inside the app itself on the protected hub, and
          extended with version filtering or custom aggregations. Building the
          pipeline is also the more interesting part: knowing what{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            sendBeacon
          </code>{" "}
          does and why, how to store and aggregate percentile data in Postgres,
          how to wire a collection client to a backend — that&apos;s the stuff
          worth knowing as a developer.
        </p>
      </section>
      ...
      <section>
        ...
        <p className="text-muted">
          ...
          npm package hooks into browser APIs to detect each metric at the right
          time and fires a callback with the name, value, and a rating. A{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            WebVitalsReporter
          </code>{" "}
          client component in the root layout registers all five collectors once
          on mount and sends each metric to{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            /api/vitals
          </code>
        ...
        <p className="mt-3 text-muted">
          ...
          content type. Regular fetch gets killed when the browser tears down
          the page — INP and CLS fire on page hide, which is exactly when a
          regular fetch would be cancelled. The Blob wrapper forces the content
          type that Express&apos;s JSON parser expects; sendBeacon defaults to{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            text/plain
          </code>{" "}
          otherwise.
        ...
        <ul className="mt-2 space-y-2 text-muted">
          <li className="flex gap-2">
            ...
            <span>
              ...
              Contentful Paint. How long until the biggest element is visible.
              Good under 2.5s. The main perceived load speed metric.
            </span>
          </li>
          <li className="flex gap-2">
            <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-foreground/30" />
            <span>
              ...
              Contentful Paint. Time until any content shows up at all. Good
              under 1.8s.
            </span>
          </li>
          <li className="flex gap-2">
            <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-foreground/30" />
            <span>
              <strong className="text-foreground">INP</strong> — Interaction to
              Next Paint. How quickly the page responds to clicks, taps, and key
              presses. Replaced FID in 2024. Good under 200ms.
            </span>
          </li>
          <li className="flex gap-2">
            <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-foreground/30" />
            <span>
              <strong className="text-foreground">CLS</strong> — Cumulative
              Layout Shift. A score for how much content jumps around while the
              page loads. Under 0.1 is good.
            </span>
          </li>
          <li className="flex gap-2">
            <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-foreground/30" />
            <span>
              <strong className="text-foreground">TTFB</strong> — Time to First
              Byte. How long the browser waits for the server to start sending a
              response. Good under 800ms.
            </span>
          </li>
        </ul>
      </section>
      ...
      <section>
        ...
        <p className="text-muted">
          Averages hide the tail. If 90% of page loads take 1.2s and 10% take
          8s, the average might look fine at 1.9s. P75 means 75% of users had a
          load time at or below that number — sensitive enough to catch real
          problems without being dominated by a single extreme outlier. Google
          uses P75 for the official Core Web Vitals thresholds in search
          ranking. The by-page table filters to pages with at least 5 samples —
          one data point isn&apos;t a distribution.
        </p>
      </section>
      ...
      <section>
        ...
        <p className="text-muted">
          The Vitals landing section now has an interactive speedometer GLB. The
          needle animates from a resting &ldquo;slow&rdquo; position to the
          &ldquo;good&rdquo; zone when the section enters the viewport, using a
          frame-by-frame lerp in{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            useFrame
          </code>
          . The three primary metrics — LCP, INP, and CLS — are displayed as
          animated stat cards below the speedometer with spring-driven value
          counters and score bars. The raw GLB had bounding-box coordinates
          spanning tens of thousands of units, so a{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            Box3
          </code>{" "}
          auto-fit runs in{" "}
      ...
      <section>
        <h2 className="mb-3 text-lg font-bold">Version selector grouping</h2>
        <p className="text-muted">
          The original version selector was a flat dropdown of every patch
          version — fine with 10 versions, unusable with 50. The new selector
          groups versions into tiers: &ldquo;Current Major&rdquo; (all data in
          the major version, the default), &ldquo;Current Minor&rdquo; (all
          patches in the latest minor), the last 3 minor versions with each
          patch shown individually in optgroups, and everything older collapsed
          into one entry per minor version with aggregated data.
        </p>
        <p className="mt-3 text-muted">
          ...
          on the first segment, minor matches both first and second, and exact
          does a straight equality check. The trend chart adapts too: minor mode
          returns up to 30 patch versions so you see the full progression within
          a minor, while major mode caps at 10.
        </p>
      </section>
      ...
      <section>
        ...
        <p className="text-muted">
          ...
          with explicit property lists across the codebase and wrapping the
          hub&apos;s mount animation update in{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            startTransition
          </code>
          ...
          elements from LCP consideration entirely. <strong>TTFB</strong> on the
          hub improved by making{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            page.tsx
          </code>{" "}
          a plain sync component with no auth calls, so Next.js can statically
          pre-render it — TTFB drops from ~2.1s to ~50ms. <strong>CLS</strong>{" "}
          on the vitals page improved by extracting a{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            CHART_CONTAINER_HEIGHT
          </code>{" "}
          constant shared by both the skeleton div and the real chart wrapper so
          they reserve the same space.
        </p>
      </section>
      ...
      <section>
        ...
        <p className="text-muted">
          For a long time this dashboard sat behind a login, and this write-up
          above describes it that way — the page redirected signed-out visitors,
          and the BFF routes forwarded a JWT that the backend&rsquo;s{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            checkJwt
          </code>{" "}
          verified like any protected route. That never sat right with me,
          because the data here isn&rsquo;t personal: it&rsquo;s the site-wide
          P75 for every visitor&rsquo;s page loads, aggregated. There&rsquo;s
          nothing in it that belongs to any one account, so gating it only made
          it harder to look at and easy to describe inconsistently — the menu
          link showed only when you were logged in, half the copy called it
          &ldquo;protected,&rdquo; and the other half called it
          &ldquo;site-wide.&rdquo;
        </p>
        <p className="mt-3 text-muted">
          So Web Vitals is public now, and it is not auth-gated anywhere. The
          proxy no longer redirects{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            /vitals
          </code>{" "}
          ...
          . The page and the BFF routes still forward a token when the visitor
          has one, but they no longer require it — a signed-out request just
          goes through unauthenticated, and the menu shows the link to everyone.
          The one thing that has to follow is the backend:{" "}
          <code className="rounded bg-surface px-1 py-0.5 text-[13px] font-mono text-foreground">
            /api/vitals/summary
          </code>{" "}
          and its siblings still verify the JWT, so until that side opens up
          too, a signed-out visitor sees the dashboard shell with the numbers
          empty rather than the full picture. Graceful, but the real fix is both
          halves.
        </p>
      </section>
      <Update
