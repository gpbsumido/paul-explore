    ...
    <ThoughtLayout
      ...
      intro={
        <>
          ...
          is a low-poly downtown at night — real streets, real landmarks — where
          each feature of this site gets an exhibit at the landmark that suits
          it: Web Vitals on the Yonge-Dundas billboards, the design system at
          OCAD, fantasy NBA at the arena. WASD to walk, E to step into a
          feature. This is how a &ldquo;game&rdquo; fits into a codebase that
          insists on TDD and pure functions.
        </>
      }
    >
      <UpdateTimeline
      ...
      <Section title="The game is a pure function; the scene is a shell">
        <p>
          ...
          <C>src/lib/world</C> as plain data-in data-out functions: <C>input</C>{" "}
          turns held keys into a normalized move vector, <C>movement</C>{" "}
          integrates one frame of acceleration, friction, turning, and bounds,{" "}
          <C>colliders</C> resolves a circle against building footprints with
          sliding, and <C>proximity</C> answers &ldquo;which exhibit is the
          player standing at.&rdquo; The R3F component calls <C>stepPlayer</C>{" "}
          once per frame and copies the result onto three.js objects —
          that&rsquo;s its whole job.
        </p>
        <p className="mt-3">
      ...
      <Section title="Toronto out of boxes">
        <p>
          The city is data first: a street grid named after the real one (Yonge,
          Queen, Spadina, University, Front) and hand-placed landmark plots,
          then a seeded generator fills the blocks between roads with towers.
          The seed is 416, obviously. Because generation is index-based and
          deterministic, the skyline is identical on every load — and
          snapshot-tested.
        </p>
        <ul className="mt-3 space-y-2">
          <Bullet>
            Landmarks are bespoke primitive builds: the CN Tower&rsquo;s pod and
            blinking beacon, City Hall&rsquo;s two curved half-pipe towers
            around the saucer, OCAD&rsquo;s checkered tabletop on colored stilts
            (no collider — you can walk under it, like in life), the Gooderham
            flatiron as an extruded wedge, a TTC streetcar shuttling along
            Queen.
          </Bullet>
          <Bullet>
            Integrity tests keep the fiction honest: every exhibit&rsquo;s{" "}
            <C>featureId</C> must exist in <C>FEATURES</C>, stand on ground the
            collision system says is reachable, sit inside the world bounds, and
            keep enough distance from its neighbours that two placards can never
            fight over the player.
          </Bullet>
          <Bullet>
      ...
      <Section title="Keeping 60fps honest">
        <p>
          ...
          dozen idle <C>useFrame</C> callbacks cost a distance check each. There
          are exactly two real lights plus a lantern point light that follows
          the player; shadows are faked with blob circles. The canvas pauses
          entirely if it scrolls out of view, and reduced-motion turns off every
          ambient animation while leaving the world fully walkable.
        </p>
      </Section>
      ...
      <Section title="The toys on top">
        <p>
          ...
          computed from local time with night as the fallback. A fidelity slider
          scales every curve from chunky low-poly to very smooth (it feeds a
          context inside the canvas that geometry segment counts read). The
          explorer has a wardrobe — Jays, Raptors, Tempo, and a silver number 1
          — plus a jump with real gravity in the tested core. And the exhibit
          list doesn&rsquo;t teleport: it speed-runs the character through the
          actual street grid, planned by a tiny road-graph router, before
          opening the feature.
        </p>
        <p className="mt-3">
          ...
          exhibits along real streets. It&rsquo;s the multiplayer-shaped feature
          with zero infrastructure: the recording throttle, snapshot shape, and
          replay interpolation are exactly the pieces a live presence layer
          would reuse.
        </p>
        <p className="mt-3">
          ...
          after five silent seconds. Without a key, your own other tabs count as
          company. It all sits behind a real flag in the{" "}
      ...
      <Section title="Reasons to keep walking">
        ...
        <p className="mt-3">
          ...
          building footprints and occasionally lose the explorer behind a tower;
          now it samples its own sightline and shortens the boom until
      ...
      <Section title="Whatever it's doing outside">
        <p>
          ...
          has been using it for ages — so the world just asks the same hook and
          dresses accordingly. Rain goes straight down, snow wanders, fog pulls
          the skyline in close, and a thunderstorm gets a light that flashes on
          an irregular timer. The particle budget rides the fidelity slider, and
          the whole field follows the camera so a few hundred points cover a
          whole city.
        </p>
        <p className="mt-3">
          Photo mode is the other half: <C>P</C> parks the simulation, clears
          the HUD, and hands the camera over to orbit freely. The shutter is the
          interesting part — instead of paying the cost of a preserved drawing
          buffer on every frame forever, it renders one frame synchronously and
          reads it straight back out before the browser clears it.
        </p>
      </Section>

      <Section title="A costume you have to earn">
        <p>
          Three of the twenty-five tokens float above the street, and the fourth
          outfit in the picker is a locked <C>???</C>. Collect every token you
          can reach on foot — pointedly not those three — and the mystery turns
          out to be a very tall man. Put him on and the last three are simply
          within reach, no jumping required. The whole loop is four small pure
          functions and it took longer to describe than to test.
        </p>
        <p className="mt-3">
      ...
      <Section title="Sound with nothing to download">
        <p>
          ...
          the repo. Traffic is low-passed noise with a slow swell on the cutoff;
          the lake is the same noise through a band-pass breathing in and out;
          rain is brighter and denser; the streetcar is two detuned saws behind
          a filter, which turns out to be most of the way to an electric motor.
          Footsteps and jumps are short noise bursts with an exponential decay,
          and picking up a token is a triangle sweep.
        </p>
        <p className="mt-3">
          ...
          thing. Browsers rightly refuse to let a page make noise unprompted, so
          the audio context isn&rsquo;t created until you actually press
      ...
      <Section title="Three bands on a phone">
        <p>
          ...
          menu sat on top of both. Widths that never touch on a laptop all land
          in the same place on a phone.
        </p>
        <p className="mt-3">
          ...
          the joystick on one thumb and <C>E</C> and Jump on the other.
          Everything between the two belongs to the cards. Tapping the toggle
          slides the whole rail in over the city and dims it; tapping the city
          puts it back.
        </p>
        <p className="mt-3">
      ...
      <Section title="A 3D page that screen readers can use">
        <p>
          ...
          shortcut, and the minimap is labeled SVG built from the same road data
          as the world. Touch devices get a pointer-events joystick instead of
          the keyboard legend.
        </p>
      </Section>
      <Update
