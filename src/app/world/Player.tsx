 ...
 */
export default function Player({ outerRef, bobRef, motionRef, lantern, outfit }: PlayerProps) {
  ...
  useFrame(() => {
    ...
    if (leftArmRef.current) leftArmRef.current.rotation.x = airborne ? -0.9 : swing;
    if (rightArmRef.current) rightArmRef.current.rotation.x = airborne ? -0.9 : -swing;
    if (leftLegRef.current) leftLegRef.current.rotation.x = airborne ? 0.55 : -swing * 0.9;
    if (rightLegRef.current) rightLegRef.current.rotation.x = airborne ? 0.4 : swing * 0.9;
    ...
    if (shadow) {
      ...
      (shadow.material as THREE.MeshBasicMaterial).opacity = 0.35 * Math.max(0.35, 1 - motion.air * 0.12);
    }
  });
  ...
  return (
    <group ref={outerRef}>
      ...
      <group ref={bobRef}>
        ...
        {/* rosy cheeks */}
        {[-1, 1].map((side) => (
          <mesh key={side} position={[side * 0.16, 1.27, 0.2]} scale={[1, 0.7, 0.5]}>
        ...
        <mesh position={[0, 1.46, -0.01]}>
          <sphereGeometry args={[0.275, fine, Math.ceil(fine * 0.6), 0, Math.PI * 2, 0, Math.PI * 0.52]} />
        ...
        <mesh position={[0, 1.47, 0.33]} rotation={[-0.25, 0, 0]} scale={[1, 0.22, 1.15]}>
          <cylinderGeometry args={[0.19, 0.19, 0.16, round, 1, false, -Math.PI / 2, Math.PI]} />
