...
const renderSheet = (props: Partial<Parameters<typeof WorldHudSheet>[0]> = {}) =>
  render(
    <WorldHudSheet summary="🪙 3/25 · 12%" menu={<button type="button">Site menu</button>} {...props}>
      <p>rail contents</p>
    </WorldHudSheet>,
  );
...
describe("WorldHudSheet", () => {
  ...
  it("starts collapsed and points its toggle at the panel it controls", () => {
    ...
    expect(within(panel as HTMLElement).getByText("rail contents")).toBeInTheDocument();
  });

  it("shows the collected-token summary on the toggle so it stays glanceable", () => {
    renderSheet();

    expect(screen.getByRole("button", { name: /world settings/i })).toHaveTextContent(
      "🪙 3/25 · 12%",
    );
  });
  ...
  it("expands when the toggle is pressed", async () => {
    ...
    expect(screen.getByRole("button", { name: /close world settings/i })).toHaveAttribute(
      "aria-expanded",
      "true",
    );
  });
  ...
  it("collapses again on a second press", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /world settings/i }));
    await user.click(screen.getByRole("button", { name: /close world settings/i }));

    expect(screen.getByRole("button", { name: /world settings/i })).toHaveAttribute(
      "aria-expanded",
      "false",
    );
  });
  ...
  it("offers no toggle at all in photo mode", () => {
    ...
    expect(screen.queryByRole("button", { name: /world settings/i })).not.toBeInTheDocument();
  });
